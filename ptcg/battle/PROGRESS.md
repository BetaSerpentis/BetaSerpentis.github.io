...
## 优化与查错（2026-09-19 第四轮，用户报告 4 项）

- [x] **选择宝可梦的列表信息过多**（卡牌页签 → 能量 → 选择附着目标）
      修复：统一为与【场地】列表一致的格式「名字 + 当前HP/最大HP·能量简写」
      同时统一了另外两处同类列表（撤退选择、效果选目标），去掉「（出战）/（备战N）/HP x/y · 能量 n」等冗余
- [x] **「令这只宝可梦昏厥」类特性自爆后不离场**（彷徨夜灵「咒怨炸弹」）
      根因：`EffectExecutor.knockout` 把**宝可梦对象**传给了接收**玩家**的 `GameState.knockout(pl)`，
            内部读 `pl.active` 得到 undefined 后直接 return → 只把 HP 设为 0，不换人、不进弃牌区、不给奖赏
      修复：改为传玩家并走 `_knockoutPokemon`（正确区分出战/备战，无后备时判负）
- [x] **当回合不可进化的宝可梦仍可点「进化」**
      修复：`_showCardActions` 的进化项按「名字匹配 + 未出场 + 未进化」判断并置灰，并提示「本回合刚出场或已进化，下回合才能进化」
      同时 `_pickPokemonFor` 的进化目标列表也对不可选目标置灰
- [x] **同名竞技场：经实测属规则允许，非 bug**
      实测：同名竞技场**卡**无法再打出（`stadium_same_name`）✓；同一玩家同回合无法重复发动同一竞技场**效果**（`already_used`）✓；
            而「发动当前竞技场效果」按规则双方每回合各可用一次 → 对手发动是合法的
      改进：为避免把「发动效果」误读成「打出竞技场卡」，日志改为「发动了竞技场「X」的效果」
