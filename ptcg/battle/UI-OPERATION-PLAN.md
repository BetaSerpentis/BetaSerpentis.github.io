...
### 已完成（Step B2 死代码清理 + 4 项体验优化）

- **体验优化**（按反馈）
  - 列表项改为单行：卡名左对齐、类型/元信息右对齐（`.menu-item` flex + `.mv-name`/`.mv-meta`），条目更薄（padding 5px）
  - 动作完成后**留在来源页签**：卡牌动作 → 回手牌列表；场地动作（特性/附能/装备/进化/撤退）→ 回场地列表（新增 `_returnView` + `_goBackToList`）
  - 主菜单「宝可梦」改名「**场 地**」
  - 场地动作子菜单移除重复的「返回宝可梦」项（保留底部 `← 返回`）
  - 支援者等训练家项改用 `GameState.canUseTrainer` 统一合法性判定（先攻首回合/每回合 1 次/使用前提都会正确禁用并显示原因）
- **Step B2 死代码清理**
  - 删除 `main.js` 中 `[DEPRECATED]` 旧全屏卡牌/宝可梦界面与 legacy 选卡/目标面板方法（约 700 行）；`main.js` 1157 → 936 行
  - 删除 `#panel-target` DOM 与其事件绑定；`_onEngineLog`/`_syncPlayerMainPanel` 中的死界面判断清理
  - 测试同步：移除 13 个旧界面测试块，新增 4 个操作区路径测试（支援者禁用、物品使用后留在卡牌列表、布置阶段放置选项、场地撤退项与去返回项）；修复 1 个洗牌随机性 flaky 断言
  - 结果：**263 项自动化测试全绿**（原 272 项 - 13 旧 + 4 新）
- **初始布置改走新流程**：开局直接显示手牌列表（点卡→放置战斗区/备战区），列表底部提供「确认布置」；起手无基础时由主菜单「重新抽牌」处理

### 待办

- **真机走查**：手牌/场地/动作子菜单/效果选卡/撤退支付在 iPhone 尺寸下的滚动与点击体验
- 可选：精灵图本地化（纯本地运行）、操作区列表项轻量上下文提示
